...
const seedDB = async () => {
  try {
    ...
    // Capture Student IDs for linking
     const s1 = await User.create({ name: "John Doe", email: "john@example.com", password: pass, role: "user" });
    const s2 = await User.create({ name: "Jane Smith", email: "jane@example.com", password: pass, role: "user" });
