...
import { X, Upload, Loader2, CheckCircle, CreditCard, AlertCircle } from 'lucide-react';
...
const BookingModal = ({ isOpen, onClose, course }: BookingModalProps) => {
  ...
  const [formData, setFormData] = useState({
    fullName: '',
    email: '',
    confirmEmail: '', // New field for verification
    phone: '',
    address: '',
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-slate-900 w-full max-w-lg rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
          ...
          <button onClick={onClose} className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
        ...
        <div className="p-6 max-h-[80vh] overflow-y-auto">
          {step === 1 ? (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div key={f.name}>
                    ...
                    <input
                      required name={f.name} type={f.type} value={(formData as any)[f.name]}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border ${formData.email && !isValidEmail(formData.email) ? 'border-rose-500' : 'border-slate-200 dark:border-slate-700'} focus:ring-2 focus:ring-indigo-500 outline-none dark:text-white text-sm font-semibold`}
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Confirm Email</label>
                  <input
                    required name="confirmEmail" type="email" value={formData.confirmEmail}
                    onChange={handleChange} placeholder="Repeat email"
                    className={`w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border ${formData.confirmEmail && formData.email !== formData.confirmEmail ? 'border-rose-500' : 'border-slate-200 dark:border-slate-700'} focus:ring-2 focus:ring-indigo-500 outline-none dark:text-white text-sm font-semibold`}
                  />
                </div>
              </div>
                  ...
                  <>
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <input
                          required name="transactionId" value={formData.transactionId}
                      ...
                      <div className="relative border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl p-5 text-center hover:border-indigo-400 transition-colors cursor-pointer group">
                        ...
                        <div className="flex flex-col items-center gap-2 text-slate-400 group-hover:text-indigo-500 transition-colors">
                          {file ? <CheckCircle className="text-emerald-500" size={28} /> : <Upload size={22} />}
                          <span className="text-[11px] font-black uppercase tracking-widest">{file ? file.name : 'Upload Payment Screenshot'}</span>
                        </div>
                      </div>
                    </div>
                  </>
              ...
              {/* Error Alert */}
              {error && (
                <div className="flex items-center gap-2 text-xs font-bold text-rose-500 bg-rose-50 dark:bg-rose-900/20 px-4 py-2.5 rounded-xl border border-rose-100 dark:border-rose-800/30">
                  <AlertCircle size={14} />
                  {error}
                </div>
              )}
              ...
              >
                {loading ? <><Loader2 size={16} className="animate-spin" /> Processing…</> : isOnline ? 'Pay Now Securely' : 'Confirm Booking'}
              </button>
            </form>
          ) : (
            <div className="text-center py-8">
              ...
              <button onClick={onClose} className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest hover:opacity-90 transition-opacity">
                Close
              </button>
            </div>
          )}
